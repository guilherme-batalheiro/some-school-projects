...
Cypress.Commands.add('checkStudentStats', (numStudents, numAtLeast75, numMoreThan3) => {
  cy.get('.container').contains('Statistics for this course execution');
  cy.get('[data-cy="totalStudents"]').contains(numStudents);
  cy.get('[data-cy="atLeast75"]').contains(numAtLeast75);
  cy.get('[data-cy="moreThan3"]').contains(numMoreThan3);
});

Cypress.Commands.add('checkQuizStats', (totalQuizzes, uniqueQuizzes, averageQuizzes) => {
  cy.get('.container').contains('Statistics for this course execution');
  cy.get('[data-cy="totalQuizzes"]').contains(totalQuizzes);
  cy.get('[data-cy="uniqueQuizzes"]').contains(uniqueQuizzes);
  cy.get('[data-cy="averageQuizzes"]').contains(averageQuizzes);
});

Cypress.Commands.add('checkQuestionStats', (totalQuestions, uniqueQuestions, averageQuestions) => {
  cy.get('.container').contains('Statistics for this course execution');
  cy.get('[data-cy="totalQuestions"]').contains(totalQuestions);
  cy.get('[data-cy="uniqueQuestions"]').contains(uniqueQuestions);
  cy.get('[data-cy="averageQuestions"]').contains(averageQuestions);
});

Cypress.Commands.add('compareScreenshots', (expected, received) => {
  // PNGJS lets me load the picture from disk
  const PNG = require('pngjs').PNG;
	// pixelmatch library will handle comparison
  const pixelmatch = require('pixelmatch');

  cy.readFile(`./tests/e2e/base-screenshots/expected/${expected}.png`, 'base64').then(baseImage => {
		cy.readFile(`./tests/e2e/screenshots/dashboard/teacherDashboard.js/${received}.png`, 'base64').then(receivedImage => {
		  // load both pictures
		  const img1 = PNG.sync.read(Buffer.from(baseImage, 'base64'));
		  const img2 = PNG.sync.read(Buffer.from(receivedImage, 'base64'));
