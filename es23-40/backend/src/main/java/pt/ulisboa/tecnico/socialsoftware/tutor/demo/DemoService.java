...
public class DemoService {
    ...
    public void resetDemoDashboards() {
        ...
        Set<DifficultQuestion> difficultQuestionsToRemove = courseExecutionRepository
                .findById(courseExecutionService.getDemoCourse().getCourseExecutionId()).stream()
    ...
    public void resetDemoDiscussions() {
        List<Discussion> discussions = discussionRepository
                .findByExecutionCourseId(courseExecutionService.getDemoCourse().getCourseExecutionId());
    ...
    public void resetDemoAnswers() {
        ...
        Set<QuizAnswer> quizAnswers = quizAnswerRepository
                .findByExecutionCourseId(courseExecutionService.getDemoCourse().getCourseExecutionId());

        for (QuizAnswer quizAnswer : quizAnswers) {
            if (!quizAnswer.getQuiz().getId().equals(quiz2Keep)
                    || !quizAnswer.getStudent().getUsername().equals(DemoUtils.STUDENT_USERNAME)) {
                quizAnswer.remove();
    ...
    public void resetDemoTournaments() {
        tournamentRepository
                .getTournamentsForCourseExecution(courseExecutionService.getDemoCourse().getCourseExecutionId())
                .forEach(tournament -> {
    ...
    public void resetDemoQuestionSubmissions() {
        questionSubmissionRepository
                .findQuestionSubmissionsByCourseExecution(courseExecutionService.getDemoCourse().getCourseExecutionId())
    ...
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public void resetDemoCourseExecutions() {
    ...
    public void populateDemo() {
        ...
        softwareArchitectureTopic.setCourse(courseRepository.findById(courseId)
        ...
        softwareEngineeringTopic.setCourse(courseRepository.findById(courseId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_NOT_FOUND)));
        ...
        assessment.setCourseExecution(courseExecutionRepository.findById(courseExecutionId)
        ...
        inClassOneWayQuiz.setCourseExecution(courseExecutionRepository.findById(courseExecutionId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_EXECUTION_NOT_FOUND)));
        ...
        inClassQuiz.setCourseExecution(courseExecutionRepository.findById(courseExecutionId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_EXECUTION_NOT_FOUND)));
        ...
        proposedQuiz.setCourseExecution(courseExecutionRepository.findById(courseExecutionId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_EXECUTION_NOT_FOUND)));
        ...
        scrambledQuiz.setCourseExecution(courseExecutionRepository.findById(courseExecutionId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_EXECUTION_NOT_FOUND)));
    ...
    public void populateDemoTest() {
        ...
        CourseExecution demoExecution = courseExecutionRepository.findById(courseExecutionId)
                .orElseThrow(() -> new TutorException(ErrorMessage.COURSE_EXECUTION_NOT_FOUND));
        demoExecution.setEndDate(DateHandler.toLocalDateTime("2024-12-31T10:15:30+01:00[Europe/Lisbon]"));

        // Simulate login of demo teacher (this adds the demo teacher to the original
        // demo execution
        ...
        List<String> endDates = Arrays.asList(
                ...
                "2025-12-31T10:15:30+01:00[Europe/Lisbon]");
        List<String> academicTerms = Arrays.asList("1st semester 2021/2022", "1st semester 2022/2023",
        ...
        for (int i = 0; i < endDates.size(); i++) {
            ...
            Student student = new Student("jola" + i, "jola1" + i, "jola@gmail.com" + i, false,
                    AuthUser.Type.TECNICO);
            student.addCourse(newCE);
        ...
        Student student = new Student("joao", "joao1", "joao@gmail.com", false,
                AuthUser.Type.TECNICO);
        student.addCourse(demoExecution);
