...
void state_init() {
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        ...
        /* init all locks in files */
        pthread_mutex_init(&(open_file_table[i]).mutex_lock, NULL);
    }

    pthread_mutex_init(&mutex_lock_free_blocks, NULL);
}

void state_destroy() { 
    pthread_mutex_destroy(&mutex_lock_free_blocks);
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_destroy(&(open_file_table[i]).mutex_lock);
    }
}
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                /* Set all ids of the blocks to -1 */
                for (int i = 0; i < INODE_BLOCK_NUMBER + 1; i++) {
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_size > 0) {
        for (int i = 0; i < inode_table[inumber].i_size / BLOCK_SIZE + 1; i++)
...
int find_in_dir(int inumber, char const *sub_name) {
    ...
    pthread_rwlock_wrlock(&inode_table[inumber].rwlock);
    /* Locates the block containing the directory's entries */    
    int block_index = inode_table[inumber].i_data_blocks[0];
    dir_entry_t *dir_entry = (dir_entry_t *)data_block_get(block_index);
    if (dir_entry == NULL) {
        pthread_rwlock_unlock(&inode_table[inumber].rwlock);
        return -1;
    }
...
int data_block_alloc() {
    pthread_mutex_lock(&mutex_lock_free_blocks);
    for (int i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_mutex_unlock(&mutex_lock_free_blocks);
            return i;
        }
    }
...
void *data_block_get(int block_number) {
    ...
    void *data = &fs_data[block_number * BLOCK_SIZE];
...
int add_to_open_file_table(int inumber, size_t offset) {
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_lock(&open_file_table[i].mutex_lock);
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_mutex_unlock(&open_file_table[i].mutex_lock);
            return i;
        }
        pthread_mutex_unlock(&open_file_table[i].mutex_lock);
    }
...
int remove_from_open_file_table(int fhandle) {
    ...
    pthread_mutex_lock(&open_file_table[fhandle].mutex_lock);
    free_open_file_entries[fhandle] = FREE;
    pthread_mutex_unlock(&open_file_table[fhandle].mutex_lock);
