...
int open_files = 0;
int destroy_after_close_is_running = 0;
...
int tfs_init() {
    ...
    if (pthread_mutex_init(&destroy_mutex, 0) != 0)
        return -1;
    ...
    if (pthread_cond_init(&all_files_closed, NULL) != 0) {
        return -1;
    }

    destroy_after_close_is_running = 0;
...
int tfs_destroy() {
    ...
    if (pthread_mutex_destroy(&destroy_mutex) != 0) {
        return -1;
    }

    if (pthread_cond_destroy(&all_files_closed) != 0) {
        return -1;
    }
...
int tfs_destroy_after_all_closed() {
    destroy_after_close_is_running = 1;
    if (pthread_mutex_lock(&destroy_mutex) != 0)
        return -1;
    while (open_files > 0) {
        pthread_cond_wait(&all_files_closed, &destroy_mutex);
    }
    if (pthread_mutex_unlock(&destroy_mutex) != 0)
        return -1;
    tfs_destroy();
...
int tfs_open(char const *name, int flags) {
    if (destroy_after_close_is_running != 0)
        return -1;
    if (pthread_mutex_lock(&single_global_lock) != 0)
        return -1;
...
int tfs_close(int fhandle) {
    ...
    open_files--;
    if (pthread_cond_broadcast(&all_files_closed) != 0) {
        return -1;
    }
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;
